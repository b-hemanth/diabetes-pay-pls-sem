...
# results
main <- foot_pls$path_coefs %>% as_tibble(rownames = "var")
inner <- foot_pls$inner_model

write_csv(main, "main.csv")
write.table(inner, "inner.txt")

stargazer::stargazer(inner)

stargazer::stargazer(reg)
plspm::innerplot(foot_path, txt.col = "black")
ggsave("plssem_path.png")
innerplot(reg_path, show.values = TRUE, arr.tcol = "black", txt.col = "black")
ggsave("plssem_2stage.png")
plot(foot_pls, arr.tcol = "black", txt.col = "black")
ggsave("plssem_coefs.png")
plot(foot_pls, what = "loadings", arr.width = 0.1, arr.tcol = "black", txt.col = "black")
ggsave("plssem_fct_load.png")